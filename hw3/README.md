...
The key difference between [Policy Gradient (PG)](https://github.com/JeongHwaSik/cs285/tree/main/hw2) methods and Q-learning lies in what each approach tries to learn. Policy Gradient methods explicitly optimize a parameterized policy $\pi(a|s)$ which directly maps states into a probability distribution over actions. In contrast, Q-learning does not learn a policy explicitly. Instead, it learns an **action-value function $Q(s, a)$ (or Q-function)** which estimates the expected return of taking action $a$ in state $s$. The policy is then derived implicitly by selecting actions that maximize the Q-value: $a = arg\max\limits_{a}Q(s,a)$ and Q-learning updates its Q-values using the Bellman equation, often referred to as the Bellman backup. 

One important advantage of Q-learning is that it is an off-policy method. This means it can learn from data generated by a different policy (called the behavior policy), not necessarily the one being optimized. This off-policy nature makes Q-learning more sample-efficient and flexible in settings where data reuse or learning from stored experience (e.g., replay buffers) is desirable. To see more detailed analysis of the Q-Learning, click [🔥here🔥](https://github.com/JeongHwaSik/cs285/blob/main/hw3/hw3.pdf).

On the other hand, Policy Gradient methods are typically on-policy, requiring data collected from the current policy in use. This distinction makes Q-learning better suited for certain environments, particularly when large amounts of offline data are available or when exploration strategies (e.g., $\epsilon$-greedy) differ from the target policy (e.g., $arg\max\limits_{a}Q(s,a)$ ).

In recent years, many Model-Free reinforcement learning approaches have leveraged latent spaces for tasks such as action planning (e.g., [PlaNet](https://arxiv.org/pdf/1811.04551), [Dreamer](https://arxiv.org/pdf/1912.01603)). This trend reflects the advantages of representing complex features and behaviors more effectively in latent space. See PlaNet implementation in [Experiment 3]().
...
[DQN](https://github.com/JeongHwaSik/cs285/tree/main/hw3#experiment-1-deep-q-learning-dqn) performs well in environments with discrete action spaces because computing $\max\limits_{a}Q(s,a)$ is straightforward. However, in continuous action spaces, finding this maximum becomes a challenging optimization problem, often non-linear and non-convex. To address this, [(Soft) Actor-Critic](https://arxiv.org/pdf/1801.01290) methods use two separate networks: one to approximate the Q-function (similar to DQN) and another to represent the policy $\pi$ that is explicitly trained to maximize the expected Q-value, $E_{a\sim\pi(a|s)}Q(s,a)$. Unlike DQN, which relies on $\epsilon$-greedy exploration, SAC encourages exploration by maximizing entropy. (For a deeper explanation of entropy maximization in the context of reinforcement learning and control as probabilistic inference, refer to this [paper](https://arxiv.org/pdf/1805.00909)). SAC also provides two main techniques for updating the policy $\pi$: the **REINFORCE gradient estimator** and the **REPARAMETRIZATION trick**.
...
## Experiment 4: Latent Space Model-Free RL

Latent space model is totally based on variation inference, which is widely used in [VAE](https://arxiv.org/pdf/1312.6114) or [beta-VAE](https://openreview.net/pdf?id=Sy2fzU9gl).


(needs to implement [PlaNet](https://arxiv.org/pdf/1811.04551)...)