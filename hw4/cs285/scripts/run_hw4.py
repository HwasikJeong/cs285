...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        next_ob = np.stack(next_ob, axis=0).mean(axis=0)
...
def run_training_loop(
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(config["num_agent_train_steps_per_iter"], dynamic_ncols=True):
            ...
            # (theoretically necessary, but in practice not because of SGD and random init.)
            # Use `replay_buffer.sample` with config["train_batch_size"].
            for i in range(mb_agent.ensemble_size):
                train_samples = replay_buffer.sample(config['train_batch_size'])
                loss = mb_agent.update(
                    i=i, 
                    obs=train_samples['observations'], 
                    acs=train_samples['actions'],
                    next_obs=train_samples['next_observations']
                )
