...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        norm_obs_acs = (obs_acs - self.obs_acs_mean) / (self.obs_acs_std + 1e-6) # prevent NaN (divide-by-zero)
        norm_obs_delta = (obs_delta - self.obs_delta_mean) / (self.obs_delta_std + 1e-6)

        # train i-th dynamics model
    ...
    def get_dynamics_predictions(
        ...
        # normalizing inputs!
        obs_acs = torch.cat((obs, acs), dim=-1)
        norm_obs_acs = (obs_acs - self.obs_acs_mean) / (self.obs_acs_std + 1e-6)
        pred_norm_obs_delta = self.dynamics_models[i](norm_obs_acs)
        pred_obs_detlta = pred_norm_obs_delta * self.obs_delta_std + self.obs_delta_mean
        pred_next_obs = obs + pred_obs_detlta
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        for acs in np.moveaxis(action_sequences, 1, 0):
            ...
            for i, ob in enumerate(obs):
                pred_next_ob = self.get_dynamics_predictions(i, ob, acs)
                next_obs.append(pred_next_ob)
            ...
            rewards = []
            for next_ob in next_obs:
                reward, _ = self.env.get_reward(next_ob, acs)
    ...
    def get_action(self, obs: np.ndarray):
        """
        Choose the best action using model-predictive control (MPC).
        ...
        if self.mpc_strategy == "random": 
            ...
            rewards = self.evaluate_action_sequences(obs, action_sequences)
            topk_indices = np.argpartition(rewards, -self.cem_num_elites)[-self.cem_num_elites:]
            elites = action_sequences[topk_indices]
            elite_mean, elite_std = np.mean(elites), np.std(elites)
            for _ in range(self.cem_num_iters):
                # TODO(student): implement the CEM (Cross-Entropy Method) algorithm
            ...
            # select optimal actions after 'self.cem_num_iters' update
            action_sequences = np.random.normal(
                elite_mean,
                elite_std,
                size=(self.mpc_num_action_sequences, self.mpc_horizon, self.ac_dim),
