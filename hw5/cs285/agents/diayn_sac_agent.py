...
class DIAYN_SAC(nn.Module):
    def __init__(
        ...
        self.discriminator = Discriminator(
            obs_dim = observation_shape[-1], 
            n_skills = n_skills, 
            n_layers = 3, # FIXME 
            hidden_dim=256 # FIXME 
        )
    ...
    def update_critic(
        ...
        z_prior = z_prior.repeat(batch_size, 1)
        ...
        with torch.no_grad():
            ...
            next_qs = self.target_critic(next_obs, next_action) # Q_{𝜙'}(s', a')
            ...
            # DIAYN don't need rewards so overlap reward
            logits = self.discriminator(obs)
            q_log_prob = nn.functional.log_softmax(logits, dim=-1)
            reward = q_log_prob.gather(-1, z.unsqueeze(-1)) - torch.log(z_prior.gather(-1, z.unsqueeze(-1)))
        ...
        # Update the critic
        # Predict Q-values
        q_values = self.critic(obs, action) # Q_{𝜙}(s, a)
        ...
        # Update discriminator network: q(z|s)
        discriminator_loss = self.discriminator.update(obs, z)

        return {
            "critic_loss": loss.item(),
            "q_values": q_values.mean().item(),
            "target_values": target_values.mean().item(),
            "discriminator_loss": discriminator_loss.mean().item()
        }
    ...
    def update(
        ...
        rewards: torch.Tensor, # DIAYN don't need rewards (unsupervised setting)
...
class Discriminator(nn.Module):
    """
    Discriminator Network q(z|s) for DIAYN
    """
    def __init__(self, obs_dim, n_skills, n_layers=3, hidden_dim=128):
        ...
        self.optimizer = torch.optim.Adam(self.logits_net.parameters(), lr=3e-4)
    ...
    def update(self, obs, zs):
        logits = self(obs)
        loss = self.loss_fn(logits, zs)
